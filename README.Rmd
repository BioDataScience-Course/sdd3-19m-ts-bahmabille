...
```{r}
library(readr)
schizo <- read_csv("data/schizo.txt")
#View(schizo)
class(schizo)
schizo_ts <- ts(schizo, start = 1.000, frequency = 1) #on met f=1 car pas précisé, une observation par jour.
schizo_ts
acf(schizo_ts) 
```

```{r}
#pour extraire un vecteur du tableau dataframe: dfdollarcollonne_a_extraire : df=dataframe
time(schizo)
plot(schizo)
cycle(schizo)
```
...
class(nottem) # Objet de type 'ts' (time series). indique le type de variable : ts ou mts pour icic. (sinon il y a dataframe, 
