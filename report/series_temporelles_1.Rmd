...
# Fonction personnalisée pour plus de confort : boxplot_by_cycle est le nom de  la fonction "function", ts est son argument, ici ce sera son seul argument. C'est pour ca que après on appelle ts dans split et dans cycle. ensuite on applique ts à nottem, donc dans ts il y aura nottem, le jeu de données. permet de ne pas devoir corriger tt le lignes de codes plus haut s'il y a des erreurs. 
...
?stat.slide
#il faut indiquer comment on veut couper, qu est ce qu on veut calculer : xcut, stat. statistiques de pennington : 0 vrais 0 ou pas: juste incapable de capturer spécimen.
