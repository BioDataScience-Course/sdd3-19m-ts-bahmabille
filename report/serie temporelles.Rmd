...
trend.test(schizo) 
# Valeur p-value = 4.946e-16, le seuil alpha est de 5%, ici la p-value est nettement plus faible donc on ne rejette H0, la différence est donc significative. 
# "Il doit exister une correlation significative entre les observations et le temps en présence d’une tendance. Mais comme les données ne sont pas indépendantes entre elles, on ne peut pas se référer à une distribution théorique. La distribution de référence est simulée en randomisant un grand nombre de fois les observations dans le temps (= technique du bootstrap)". Pas interessant de faire un simple test de correlation des serie spatio-temporelle, car d'office il devrait y en avoir. Donc le bootstrap est nettement plus important.
...
# -> aggrégation des données par an, on indique 1 car l'unite de temps dans cette serie est de 1 an.
...
#premier graphique=serie initiale. deuxieme graphique= signal lissé, troisieme=residus.
