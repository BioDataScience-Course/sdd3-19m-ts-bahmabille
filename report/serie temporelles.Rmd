...
(not_stat2 <- stat.slide(time(fire_ts), fire_ts, xmin = 1.000, deltat = 10))
...
plot(lynx) 
#une variation cyclique des données peut etre observée. Pour les mettre en évidence avec la fonction cycle, il faut d'abord changer la fréquence de la série.


#????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????

lynx_ts <- ts(lynx, start = 1821, frequency = 12) 
lynx_ts
acf(lynx_ts)
time(lynx_ts) 
...
##Statistiques glissantes
lynx.stat <- stat.slide(time(lynx_ts), lynx_ts, xmin = 1821, deltat = 10)
lynx.stat
plot(lynx.stat) 
...
demographie_ts <- ts(demographie, start = 1978, frequency = 12) 
demographie_ts
