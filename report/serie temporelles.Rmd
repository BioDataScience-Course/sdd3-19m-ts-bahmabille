...
not_stat <- stat.slide(time(schizo_ts), schizo_ts, xmin = 1, deltat = 3)
...
split(earthq_ts, cycle(earthq_ts))
# Split separe la série en fonction des cycles. $'1' = toutes les mesure du mois de janvier, $'2' = février, etc. Ici toutes les observations sont reprises dans $`1` puisqu'il n'y a pas plusieurs cycles. L'utilisation de la fonction split n'est pas justifié ici.
#"split divides the data in the vector x into the groups defined by f. The replacement forms replace values corresponding to such a division. unsplit reverses the effect of split. USAGE split(x, f...). ARGUMENTS x : vector or data frame containing values to be divided into groups. f	: a ‘factor’ in the sense that as.factor(f) defines the grouping, or a list of such factors in which case their interaction is used for the grouping."

boxplot(split(earthq_ts, cycle(earthq_ts)))
# Boites de dispersion (avec les cinqs nombres: premier quartile=q1, mediane=deuxième quartile, troisième quartile = q3, valeur minimal et valeur maximale comprises dans 1,5*IQR (interquartile range= q3-q1)) pour les données mensuelles : une boite pour chacun des cycles : une boite avec toutes les valeurs de janvier, une avec toutes les valeurs de février, etc. Ceci est possible grace aux fonctions cycles et split.
# Ici un seul boxplot est observée car il n'y a pas plusieurs cycles. L'utilisation de la fonction boxplot n'est pas justifié ici.



boxplot_by_cycle <- function(ts)
  boxplot(split(ts, cycle(ts)))
boxplot_by_cycle(earthq_ts)
#Création d'une fonction personnalisée pour plus de confort puis 3° ligne de code = pour générer les boxplots.
# Ici un seul boxplot est observée car il n'y a pas plusieurs cycles. L'utilisation de la fonction boxplot_by_cycle n'est pas justifié ici.
...
not_stat1 <- stat.slide(time(earthq_ts), earthq_ts, xmin = 1900, deltat = 10)
...
##Autocorrelation 
#comparaison avec une série purement aléatoire
x <- rnorm(99) #création d'une vecteur de nombres aléatoires
x
x_ts <- ts(x, start = 1900.000, end = 1998.000, frequency = 1) #transformation en objet ts.
acf(x_ts) #"The function acf computes (and by default plots) estimates of the autocovariance or autocorrelation function. lag.max	: maximum lag at which to calculate the acf. Default is 10*log10(N/m) where N is the number of observations and m the number of series. Will be automatically limited to one less than the number of observations in the series." 
#L'autocorrelation est directe si l'ACF vaut 1, elle est nulle si l'ACF vaut 0, l'autocorrelation est inverse si l'ACF vaut -1. Les ligne discontinues en bleu indiquent l'interval où la valeur de l'ACF est non significative. Nous pouvons observer qu'il n'y a pas d'autocorrelation exception faite de la première valeur qui a toujours une valeur de correlation de 1. 

acf(earthq_ts) #Pour earthq une faible autocorrelation positive est calculépour les 5 premières ?????? puis elle diminue jusqu'à devenir négative mais pas inverse.
...
unclass(fire) #nous voyons qu'il s'agit d'un vecteur de nombres de 1 à 70, avec comme attribut attr(,"row.names"), attr(,"spec"), cols( earthquake = col_double()).
...
plot(fire_ts) 
# Ce graphique nous montre les valeurs des differents echantillonnages allant du temps. Remarquons que les observations sont reliées les unes autres en une ligne continue pour indiquer le dépendance des observations entre elles (contrairement au nuage de points).
...
split(fire_ts, cycle(fire_ts))
# Split separe la série en fonction des cycles. $'1' = toutes les mesure du mois de janvier, $'2' = février, etc. Ici toutes les observations sont reprises dans $`1` puisqu'il n'y a pas plusieurs cycles. L'utilisation de la fonction split n'est pas justifié ici.
#"split divides the data in the vector x into the groups defined by f. The replacement forms replace values corresponding to such a division. unsplit reverses the effect of split. USAGE split(x, f...). ARGUMENTS x : vector or data frame containing values to be divided into groups. f	: a ‘factor’ in the sense that as.factor(f) defines the grouping, or a list of such factors in which case their interaction is used for the grouping."

boxplot(split(fire_ts, cycle(fire_ts)))
# Boites de dispersion (avec les cinqs nombres: premier quartile=q1, mediane=deuxième quartile, troisième quartile = q3, valeur minimal et valeur maximale comprises dans 1,5*IQR (interquartile range= q3-q1)) pour les données mensuelles : une boite pour chacun des cycles : une boite avec toutes les valeurs de janvier, une avec toutes les valeurs de février, etc. Ceci est possible grace aux fonctions cycles et split.
# Ici un seul boxplot est observée car il n'y a pas plusieurs cycles. L'utilisation de la fonction boxplot n'est pas justifié ici.
...
not_stat2 <- stat.slide(time(fire_ts), fire_ts, xmin = 1.000, deltat = 5)
...
##Autocorrelation 
#comparaison avec une série purement aléatoire
x <- rnorm(70) #création d'une vecteur de nombres aléatoires
x
x_ts <- ts(x, start = 1.000, frequency = 1) #transformation en objet ts.
acf(x_ts) #"The function acf computes (and by default plots) estimates of the autocovariance or autocorrelation function. lag.max	: maximum lag at which to calculate the acf. Default is 10*log10(N/m) where N is the number of observations and m the number of series. Will be automatically limited to one less than the number of observations in the series." 
#L'autocorrelation est directe si l'ACF vaut 1, elle est nulle si l'ACF vaut 0, l'autocorrelation est inverse si l'ACF vaut -1. Les ligne discontinues en bleu indiquent l'interval où la valeur de l'ACF est non significative. Nous pouvons observer qu'il n'y a pas d'autocorrelation exception faite de la première valeur qui a toujours une valeur de correlation de 1. 

acf(fire_ts) #Pour fire l'autocorrelation n'est pas présente..
